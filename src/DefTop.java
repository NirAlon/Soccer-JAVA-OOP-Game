...
public class DefTop extends Players {

	public DefTop(Color c, Team.sides s, int num) {
		super(c, s);
		this.num = num;
	}
	...
	public void WhatToDoWithoutBall(Ball b, SoccerField sf) {
		...
		if (T_S.equals(Team.sides.LEFT_SIDE)) {
			if(sf.getDefTopLeft().contains(b.getX(),b.getY())&&!PlayerGotBall){
			...
			else if (sf.getDefTopRight().contains(b.getX(),b.getY())&&!PlayerGotBall){
			alpha = Math.atan2(b.getY()-cy, b.getX()-cx);
